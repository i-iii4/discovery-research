...
Атрибут `profile` проставляется в frontmatter каждой гипотезы/работы прогона; `audience: b2c`, а `market` добавляется в список `pipelines`.

## Формат артефактов (единый, обогащаемый)

Все пайплайны пишут в общую поверхность `../../artifacts/` по эталонным шаблонам:
- работы → `../../artifacts/jobs/` по `../../templates/job.md`
- гипотезы → `../../artifacts/hypotheses/` по `../../templates/hypothesis.md`

**Один артефакт = одна работа/гипотеза, обогащаемая несколькими пайплайнами.** Правила:
1. **Дедуп перед записью.** Перед созданием файла проверь, нет ли уже работы/гипотезы на ту же суть в общей папке (по смыслу, не по имени). Если есть — обогащай существующий, не плоди дубль.
2. **Свой блок доказательств.** Этот пайплайн заполняет `## Доказательства — market`. Чужой блок (`## Доказательства — jtbd`) не трогать.
3. **Атрибут `pipelines`.** Добавь `market` в список (дополняй, не перезаписывай).
4. **Сводные поля.** `verdict`/`status`/`score` — сводные по всем пайплайнам; при расхождении оценок отметь это в своём блоке.

Пересечение методов — главная ценность: гипотеза, подтверждённая и market, и jtbd, — сильнейший сигнал; расхождение — повод для разбора.
