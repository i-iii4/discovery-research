...
Этот пайплайн пишет в **свою** папку `../../artifacts/market/` — отдельно от jtbd, без смешивания (два независимых проекта):
- гипотезы → `../../artifacts/market/hypotheses/` по `../../templates/hypothesis.md`
- работы (если извлекаются) → `../../artifacts/market/jobs/` по `../../templates/job.md`
- прогоны → `../../artifacts/market/runs/vN/`
- представления Bases → `../../artifacts/market/views/`

Нумерация прогонов своя, сквозная внутри пайплайна (`v1`, `v2`, …) — она не пересекается с нумерацией jtbd. Атрибуты frontmatter: `audience: b2c`, `profile`. Поле пайплайна не нужно — папка `market/` уже однозначно его задаёт.
